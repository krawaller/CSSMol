...
// default colors
var BONDS_COLOR = [255,125,0];
var ATOMS_COLOR = [255,0,0];

var BACKGROUND_COLOR = [10,10,10];
...
function parse_sdf(text) {
    ...
    try {
        ...
        return {"ok":1, "atoms":atoms, "bonds":bonds };
    }
    catch(e) {
        log(e, LOG_ERROR);
        return {"ok":0 };
...
function parse_pdb(text) {
    ...
    for(var i=0; i<lines.length; ++i) {
        if(lines[i].substr(0,4)=="ATOM" || lines[i].substr(0,6)=="HETATM" ) {
            ...
            if(ci=="") ci = trim(lines[i].substr(12,2));
            ...
            if(e1) bonds.push([s-1, e1-1, 1]);
            if(e2) bonds.push([s-1, e2-1, 1]);
            if(e3) bonds.push([s-1, e3-1, 1]);
            if(e4) bonds.push([s-1, e4-1, 1]);
        }
    }
...
function render_molecule(ctx, cx, cy, width, height, points, lines, draw_atoms, draw_bonds, bonds_autowidth, bonds_gradient, atom_colors) {
...
function Molecule(pars) {
    ...
    this.render = function() {
        ...
        render_molecule(this.ctx, this.cx, this.cy, this.width, this.height, 
                        ...
                        this.bonds_autowidth, this.bonds_gradient,
                        this.atom_colors);
    }
    ...
    this.reset_model = function() {        
        ...
        this.points = array2d(this.atoms.length, 3);
...
function molecule_div(i) {
    ...
    tmp += '<div class="button b_width">B-width</div>';
    tmp += '<div class="button b_grad">B-gradient</div>';
...
function add_molecule(label, name, text) {
    var i = M_COUNTER;
    M_COUNTER++;
    ...
    for(var r=0;r<MENDELEEV.length;++r) {
        for(var c=0;c<MENDELEEV[r].length;++c) {
            ...
            if(element) {
                ...
                var div = $("<div title='"+FULLNAME[element]+"' style='color:rgb("+CPK[element][0]+","+CPK[element][1]+","+CPK[element][2]+")'>"+name+"</div>");
                parent.append(div);
                div.css("textShadow", "0px 0px 4px rgb("+CPK[element][0]+","+CPK[element][1]+","+CPK[element][2]+")");
            }
            else
...
// Molecule menu
/*****************************************************************************************/
function create_molecule_menu(parent_id) {
    var parent =$("#"+parent_id); 
    var first = 1; 
    for(var i in MENU_MAP) {
        if(MENU_MAP[i])
    ...
    $("#"+parent_id+" li.mol").click(function() {
        var txt = $(this).text();
        add_molecule(create_permalink(txt), MENU_MAP[txt]);
